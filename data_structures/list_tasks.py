...
# b = 'в период'
# c = 'эмиграции с Терры'
# Используя только срезы с двумя или тремя параметрами и конкатенацию,
# составьте из них строку 'ты программист' (всё строчными буквами).
# Выведите результат.
# Ожидаемый вывод:
# ты программист
...
b = 'в период'
c = 'эмиграции с Терры'
you = a[a.find('ты '):a.find('ты ')+3]
pro = b[b.find('п'):b.find('п')+5:2]
gra = c[c.find('г'):c.find('г')+3]
m = c[c.find('м')]
mi = c[c.find('ми'):c.find('ми')+2]
st = a[a.find('ст'):a.find('ст')+2]
new_string = you + pro + gra + m + mi + st
print(new_string)

# Даны три списка с урожайностью трёх полей (в центнерах) за семь лет:
# field1 = [45, 50, 55, 60, 65, 70, 75]
# field2 = [45, 50, 55, 60, 65, 70, 75, 80]
# field3 = [45, 50, 55, 59, 65, 70, 75]
# Выполните следующие действия:
# Сравните field1 и field2 оператором <.
# Выведите результат и объясните (в комментарии) причину.
# Сравните field1 и field3 оператором >.
# Выведите результат и объясните причину.
# Для каждого поля найдите максимальную и
# минимальную урожайность с помощью функций max() и min().
# Выведите их с пояснениями (например: «Максимум field1: 75»).
# Ожидаемый вывод (примерный):
# field1 < field2: True
# # Объяснение: все одинаковые элементы вплоть до исчерпания field1, а field2 длиннее.
# field1 > field3: True
# # Объяснение: на индексе 3 у field1 значение 60, у field3 59, 60 > 59.
...
# Дан список овощей:
# vegetables = ['Помидоры', 'Огурцы', 'Баклажаны', 'Перец', 'Капуста', 'Кабачки', 'Редис']
# С помощью среза с отрицательным шагом получите список reversed_middle, состоящий из
# элементов с индексами от 5 до 2 (включительно), переставленных в обратном порядке.
# Проверьте с помощью оператора in, есть ли
# 'Редис' в этом инвертированном срезе. Выведите результат проверки.
# Проверьте с помощью not in, отсутствует ли
# 'Тыква' в исходном списке vegetables. Выведите результат.
# Выведите длину исходного списка и длину среза reversed_middle.
# Ожидаемый вывод (примерный):
# reversed_middle: ['Кабачки', 'Капуста', 'Перец', 'Баклажаны']
# Есть ли 'Редис' в срезе? False
# Отсутствует ли 'Тыква' в vegetables? True
# 

vegetables = ['Помидоры', 'Огурцы', 'Баклажаны', 'Перец', 'Капуста', 'Кабачки', 'Редис']
reversed_middle = f'reversed_middle: {vegetables[5:2:-1]}'
print(reversed_middle)
print(f"Есть ли 'Редис' в срезе? {'Редис' in reversed_middle}")
print(f"Отсутствует ли 'Тыква' в vegetables? {'Тыква' not in reversed_middle}")
print(f"Длина vegetables: {len(vegetables)}, длина среза: {len(reversed_middle)}")
